...
void calculateDFTMetalLargeMem(std::complex<float>* inBuffer, std::complex<float>* outBuffer, size_t num) {
    @autoreleasepool {
        id<MTLDevice> device = GetMetalSystemDevice();
        id<MTLFunction> func = initFunction(@"computeDFTMetalWithPrecomputedRoot", device);
        if (func == nil)
        {
            NSLog(@"Failed to find the DFT function.");
            return;
        ...
        // Precompute Roots
        id<MTLBuffer> bufferRoots = [device newBufferWithLength:num * sizeof(float) * 2 options:MTLResourceStorageModeShared];
        inDataPtr = (float *)bufferRoots.contents;
        for (unsigned long index = 0; index < num; index++)
        {
            std::complex<float> root = std::exp(std::complex<float>(0, -2 * M_PI * index / num));
            inDataPtr[index * 2] = root.real();
            inDataPtr[index * 2 + 1] = root.imag();
        }
        ...
        *((int *)bufferNum.contents) = (int)num;
        ...
        [computeEncoder setComputePipelineState:funcPSO];
        [computeEncoder setBuffer:bufferIn offset:0 atIndex:0];
        [computeEncoder setBuffer:bufferRoots offset:0 atIndex:1];
        [computeEncoder setBuffer:bufferOut offset:0 atIndex:2];
        [computeEncoder setBuffer:bufferNum offset:0 atIndex:3];
